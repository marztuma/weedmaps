...
export function productSchema(p, { imageUrl, rating } = {}) {
  ...
  return clean({
    ...
    /* AggregateRating ships only when `rating` is supplied, and the only
       caller supplies it from getGenuineRating(), which counts published
       reviews written by people and excludes every seeded row. So the markup
       appears the moment real reviews exist and never before — no invented
       score is ever handed to a search engine. */

...
async function getEmailConfig() {
  try {
    ...
    return {
      ...
      mailFrom: settings?.mailFrom || process.env.MAIL_FROM || null,
...
export async function mailConfigured() {
  ...
  return Boolean(config.apiKey && config.mailFrom);
}

/* Resend's shared sender. Anyone can send from it without owning a domain,
   and the trade is that it will only deliver to the address the Resend account
   itself is registered under. It is fine for proving the plumbing and useless
   for reaching a customer. */
const TEST_SENDER = "onboarding@resend.dev";

export async function mailStatus() {
  const config = await getEmailConfig();
  return {
    configured: Boolean(config.apiKey && config.mailFrom),
    hasKey: Boolean(config.apiKey),
    from: config.mailFrom,
    adminEmail: config.adminEmail,
    webhookReady: Boolean(config.webhookSecret),
    usingTestSender: Boolean(config.mailFrom && config.mailFrom.includes(TEST_SENDER)),
  };
}

/** Pull the allowed address out of Resend's restriction message.
 *
 *  The provider names the one address the shared sender may deliver to. That
 *  is the single most useful fact when diagnosing "why did nothing arrive",
 *  so it is lifted out rather than left inside a wall of error text. */
export function restrictedRecipient(error) {
  const m = /only send testing emails to your own email address \(([^)]+)\)/i.exec(String(error ?? ""));
  return m ? m[1] : null;
}
...
export async function sendMail({ template, to, subject, html, text, key, replyTo, orderId }) {
  ...
  if (!config.apiKey || !config.mailFrom) {
  ...
  try {
    ...
    try {
      result = await Promise.race([
        resend.emails.send(
          {
            from: headerSafe(config.mailFrom, 200),
            to: [recipient],
            subject: safeSubject,
            html,
            text,
...
export async function adminRecipient() {
  const config = await getEmailConfig();
  const v = normaliseEmail(config.adminEmail);

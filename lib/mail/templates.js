...
/* ── A marketing campaign ─────────────────────────────────────
   ...
   Composed here with every other message rather than inline in the action
   that sends it. Email bodies are their own medium — inline literals, absolute
   px, no custom properties — and keeping them in one file is what makes that a
   deliberate exception rather than drift leaking into the app.

   The body is operator-written and still escaped: an admin typing a stray
   angle bracket should not produce broken markup in a thousand inboxes, and
   the same escaping means a compromised admin account cannot inject script
   into a mailing list.

   The unsubscribe line is added here, not left to the caller. Marketing email
   without one is unlawful under CAN-SPAM, and a rule enforced at composition
   cannot be forgotten at a call site. */
export function campaignEmail({ body, unsubscribeUrl }) {
  const html =
    `<div style="font-family:${FONT};font-size:15px;line-height:1.6;color:${INK};max-width:560px;">` +
    escapeHtmlLines(body) +
    `<hr style="border:none;border-top:1px solid ${RULE};margin:24px 0 12px;">` +
    `<p style="font-family:${FONT};font-size:12px;line-height:1.6;color:${MUTE};">` +
    `You are receiving this because you asked for discount codes from Weedmaps. ` +
    `<a href="${escapeHtml(unsubscribeUrl)}" style="color:${MUTE};">Unsubscribe</a>.` +
    `</p></div>`;
